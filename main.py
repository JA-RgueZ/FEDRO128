...
VERSION = "1.03.006" # Versión actualizada para asegurar un cambio detectable
...
def _get_biblioteca_folder_id():
    ...
    results = drive_client.files().list(q=query, fields="files(id)").execute() # Use drive_client
...
def list_all_drive_files():
    try:
        ...
        results = drive_client.files().list(q=query, fields="files(id, name)").execute() # Use drive_client
        items = results.get('files', [])
